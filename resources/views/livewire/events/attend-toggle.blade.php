    ...
    <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 bg-white dark:bg-zinc-800">
        ...
        @if ($status === 'attending')
            <div class="mt-4 max-w-xs">
                ...
                <input type="number" min="0" wire:model.live="guestCount" @disabled($closed)
                    class="w-full text-sm border rounded-md px-3 py-2 dark:bg-zinc-700 dark:text-white" />
                <flux:button class="mt-2" size="xs" variant="ghost" wire:click="respond('attending')" :disabled="$closed">
                    Update guests
                </flux:button>
            </div>
        @endif
